...
class SearchDataProcessorDelta:
    def __init__(self):
        self.base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        self.delta_bronze_path = os.path.join(self.base_dir, "data", "raw", "bronze", "search")
        self.delta_silver_path = os.path.join(self.base_dir, "data", "processed", "silver", "search")
    ...
    def _create_spark_session(self):
        return (
            ...
            .appName("YouTubeSearchDeltaKafkaProcessing")
            .master("local[*]")
            .config(
                "spark.jars.packages",
    ...
    def process(self):
        ...
        print(f"Reading search stream from Kafka topic '{kafka_topic}'...")
        kafka_df = (
            spark.read
            .format("kafka")
        ...
        if kafka_df.rdd.isEmpty():
            raise RuntimeError(
                ...
                "Publish search records before running this processor."
            )

        parsed_df = (
            kafka_df.selectExpr("CAST(value AS STRING) AS json_str")
            .select(from_json(col("json_str"), search_schema).alias("data"))
            .select("data.*")
        )
        ...
        print("Transforming search data to Silver layer...")
        silver_df = (
            ...
            .withColumn("engagement", (col("like_count") + col("comment_count")) / when(col("view_count") > 0, col("view_count")).otherwise(lit(1.0)))
            .withColumn(
                "video_age_days",
        ...
        print("Search Delta pipeline complete (Kafka -> Bronze -> Silver).")
        spark.stop()
